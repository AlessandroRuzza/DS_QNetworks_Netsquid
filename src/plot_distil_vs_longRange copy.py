...
"""
from setup_distill_then_swap_sim import setup_distill_then_swap_sim
...
def run_distillation_sims(param_sets):
    ...
    """
    all_results = {}
    print("Running swap simulations...") 
...
def plot_distil_vs_longrange(all_res_distil, all_res_long):
    """
    Compare swap-based vs simple swap (long range):
    - PMF/CDF of attempts
    ...
    for label_distil in all_res_distil.keys():
        ...
        for idx, dist_key in enumerate(valid_distances):
            ...
            for attempts, name, color in [
                (attempts_distil, f"Distill+Swap", "C2"),  # <-- label changed
        ...
        fig_pmf_cdf.suptitle(f"Distill+Swap vs Simple Swap - PMF/CDF\n{data_distil['label_loss']}", fontsize=14)  # <-- title wording
        fig_pmf_cdf.tight_layout(rect=(0.02, 0.02, 1.0, 0.96))
        plt.savefig(get_img_path(f"Comparison_DistilFirst_vs_LongRange_PMF_CDF\n{data_distil['label_loss']}"), dpi=300, bbox_inches="tight")  # <-- filename
        plt.close()
        ...
        for idx, dist_key in enumerate(valid_distances):
            ...
            ax.set_xticks(positions, [f"Distill+Swap\n({dist_km}km)", f"Simple Swap\n({dist_km}km)"])  # <-- label changed
            ax.set_title(f"{dist_km}km", fontsize=12)
            if hasattr(ax, "grid"):
                ax.grid(True, alpha=0.3, axis='y')
        
        axes_fid[0].set_ylabel("Bell Fidelity A~C", fontsize=12)
        fig_fid.suptitle(f"Fidelity Comparison: Distill+Swap vs Simple Swap\n{data_distil['label_noise']}", fontsize=14)  # <-- title wording
        fig_fid.tight_layout(rect=(0.02, 0.02, 1.0, 0.96))
        plt.savefig(get_img_path(f"Comparison_DistilFirst_vs_LongRange_Fidelity\n{data_distil['label_noise']}"), dpi=300, bbox_inches="tight")  # <-- filename
        plt.close()


def print_comparison_table(all_res_distil, all_res_long):
    """
    Print a comparison table of distill+swap vs simple swap.
    Rows: scenario-distance pairs
    Columns: avg time units, avg fidelity for both methods
    """
    print("\n" + "="*120)
    print("COMPARISON TABLE: Distill+Swap vs Simple Swap")
    print("="*120)
    ...
    for label_distil in sorted(all_res_distil.keys()):
        ...
        for dist_key in dist_keys:
            ...
            print(f"{scenario_name:<30} {dist_key:<12} {'Distill+Swap':<20} {avg_time_distil:<18.2f} {avg_fid_distil:<15.4f}")
